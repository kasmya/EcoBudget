...
def train():
    ...
    for epoch in range(N_EPOCHS):
        ...
        # Save best weights by val success rate first, then val reward as tiebreak.
        # Success rate is the metric that matters for the poster claim.
        if (vs > best_val_success) or (vs == best_val_success and vr > best_val_reward):
            best_val_reward = vr
            best_val_success = vs
            best_epoch = epoch + 1
            best_weights = {
